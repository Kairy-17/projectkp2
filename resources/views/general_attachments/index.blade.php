...
<main class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50">
    <header class="h-16 glass flex items-center px-6 z-10 sticky top-0 border-b border-slate-200 justify-between">
        <div class="flex items-center">
            <a href="{{ route('reports.index') }}" class="text-slate-500 hover:text-indigo-600 mr-4" title="Kembali ke Laporan">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <h2 class="text-xl font-semibold">Brankas File Umum</h2>
        </div>
    </header>
        ...
        <div class="max-w-4xl mx-auto glass p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200" x-data="{ tab: 'file', search: '' }">
            ...
            <div class="bg-white p-5 rounded-xl border border-slate-200 mb-6 shadow-sm">
                ...
                <div class="flex border-b border-slate-200 mb-4">
                    <button type="button" @click="tab = 'file'" :class="tab == 'file' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500'" class="px-4 py-2 border-b-2 font-medium text-sm transition-colors">Upload File (PDF/Word/Dll)</button>
                ...
                <form action="{{ route('general-attachments.store') }}" method="POST" enctype="multipart/form-data" x-show="tab == 'file'">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Pilih File <span class="text-xs text-slate-400 font-normal">(Maks: 20MB)</span></label>
            ...
            <div class="space-y-3">
                ...
                <div class="bg-white border border-slate-100 p-4 rounded-xl shadow-sm flex items-center justify-between transition-all hover:shadow-md" x-show="search === '' || '{{ strtolower(addslashes($attachment->judul_dokumen)) }}'.includes(search.toLowerCase())">
                    <div class="flex items-center gap-4 overflow-hidden">
                        ...
                        <div class="truncate">
                            ...
                            <p class="text-xs text-slate-500 truncate">{{ $attachment->tipe == 'file' ? 'Dokumen / File Umum' : 'Tautan Eksternal' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0 ml-4">
                        ...
                        <form action="{{ route('general-attachments.destroy', $attachment->id) }}" method="POST" onsubmit="return confirm('Hapus dokumen ini?');" class="inline">
                            @csrf
                            @method('DELETE')
                ...
                <div class="text-center p-12 bg-white/50 rounded-xl border border-dashed border-slate-300">
                    ...
                    <p class="text-sm text-slate-500 font-medium">Brankas Umum masih kosong. Belum ada file/dokumen yang disimpan.</p>
                </div>
                @endforelse
            </div>
        </div>
