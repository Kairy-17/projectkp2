...
<main class="flex-1 flex flex-col h-full overflow-hidden bg-slate-50">
    <header class="h-16 glass flex items-center px-6 z-10 sticky top-0 border-b border-slate-200 justify-between">
        <div class="flex items-center">
            ...
            <h2 class="text-xl font-semibold">{{ __('Brankas File Umum') }}</h2>
        </div>
    </header>

    <div class="flex-1 overflow-y-auto p-4 sm:p-6 md:p-12">
        <!-- Brankas Umum Section -->
        <div class="max-w-4xl mx-auto glass p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200" x-data="{ tab: 'file', search: '', previewModal: false, previewUrl: '', previewType: '', previewTitle: '' }">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <svg class="w-6 h-6 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>
                    {{ __('Brankas Penyimpanan Global') }}
                </h3>
            </div>
            ...
            <div class="bg-white p-5 rounded-xl border border-slate-200 mb-6 shadow-sm">
                ...
                <form action="{{ route('general-attachments.store') }}" method="POST" enctype="multipart/form-data" x-show="tab == 'file'">
                    @csrf
                    <input type="hidden" name="tipe" value="file">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Judul Dokumen') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Pilih File') }} <span class="text-xs text-slate-400 font-normal">(Maks: 20MB)</span></label>
                            <input type="file" name="file" class="w-full rounded-lg border-slate-300 border px-3 py-1.5 text-sm file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100" required>
                        </div>
                    </div>
                    <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm">{{ __('Upload File') }}</button>
                </form>

                <form action="{{ route('general-attachments.store') }}" method="POST" x-show="tab == 'link'" x-cloak>
                    @csrf
                    <input type="hidden" name="tipe" value="link">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Judul Dokumen / Judul Link') }}</label>
                            <input type="text" name="judul_dokumen" class="w-full rounded-lg border-slate-300 border px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('URL (Tautan)') }}</label>
                    ...
                    <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm">{{ __('Simpan Link') }}</button>
                </form>
            </div>
            ...
            <div class="mb-4 relative flex items-center">
                ...
                <input type="text" x-model="search" placeholder="{{ __('Cari dokumen berdasarkan judul...') }}" class="w-full rounded-xl border-slate-200 border pl-10 pr-4 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-white/50">
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="bg-white border border-slate-100 p-4 rounded-xl shadow-sm flex items-center justify-between transition-all hover:shadow-md" x-show="search === '' || '{{ strtolower(addslashes($attachment->judul_dokumen)) }}'.includes(search.toLowerCase())">
                    <div class="flex items-center gap-4 overflow-hidden">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 {{ $attachment->tipe == 'file' ? 'bg-amber-50 text-amber-500' : 'bg-blue-50 text-blue-500' }}">
                        ...
                        <div class="truncate">
                            ...
                            <p class="text-xs text-slate-500 truncate">{{ $attachment->tipe == 'file' ? __('Dokumen / File Umum') : __('Tautan Eksternal') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0 ml-4">
                        @if($attachment->tipe == 'file')
                            ...
                            @endphp
                            <button type="button" @click="previewUrl = '{{ $isOffice ? 'https://view.officeapps.live.com/op/embed.aspx?src=' . urlencode($fileUrl) : $fileUrl }}'; previewType = '{{ $isOffice ? 'office' : 'native' }}'; previewTitle = '{{ addslashes($attachment->judul_dokumen) }}'; previewModal = true" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="{{ __('Preview File') }}">
                        ...
                        <form action="{{ route('general-attachments.destroy', $attachment->id) }}" method="POST" onsubmit="return confirm('{{ __('Hapus dokumen ini?') }}');" class="inline">
                            @csrf
                            @method('DELETE')
                ...
                <div class="text-center p-12 bg-white/50 rounded-xl border border-dashed border-slate-300">
                    ...
                    <p class="text-sm text-slate-500 font-medium">{{ __('Brankas Umum masih kosong. Belum ada file/dokumen yang disimpan.') }}</p>
                </div>
                @endforelse
            </div>
