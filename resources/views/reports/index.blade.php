...
<main class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50/50">
    ...
    <header class="h-16 glass flex items-center justify-between px-4 sm:px-6 z-40 sticky top-0">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-xl font-semibold hidden sm:block">{{ __('Laporan Kinerja') }}</h2>
        </div>
        <div class="flex items-center gap-4">
            @include('partials.notifications')
            <a href="{{ route('master-data.index') }}" class="bg-amber-100 hover:bg-amber-200 text-amber-700 px-4 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-1">
            ...
            <a href="{{ route('reports.create') }}" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-full text-sm font-medium shadow-sm shadow-emerald-200 transition-all">{{ __('+ New Report') }}</a>
        </div>
    </header>
